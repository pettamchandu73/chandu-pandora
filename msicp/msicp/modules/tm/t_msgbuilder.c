/*
 * $Id: t_msgbuilder.c,v 1.1.1.1 2006/08/31 22:40:49 hari Exp $
 *
 * message printing
 *
	...
	unsigned int *len, char *method, int method_len, str *to)
{
	...
	int branch_len;
	int nReaon_Header = 0, nLocal_Header = 0;
	str branch_str;
	struct hostport hp;
	char cRequestUri[32] = "";
	...
	*len+=Trans->uac[branch].uri.len;
	...
	/* Added for Getting local cancel branch for updating 408( missed call changes) --vravi/saidulu */
	memset(Trans->local_branch,0,sizeof(Trans->local_branch));
	snprintf(Trans->local_branch,sizeof(Trans->local_branch)-1,"%.*s",branch_len,branch_buf);
	...
	*len+=Trans->from.len+Trans->callid.len+to->len+
	...
	/*Reason*/
	if(strlen(method) == 6 && !strncmp(method,"CANCEL",6) && !Trans->nReason){
		*len += REASON_LEN + CRLF_LEN;
		nReaon_Header = 1;
	}

	memset(cRequestUri, 0, sizeof(cRequestUri));
	if(Trans != NULL && Trans->uac[branch].uri.len > 0) {
		snprintf(cRequestUri,sizeof(cRequestUri)-1,"%.*s",Trans->uac[branch].uri.len,Trans->uac[branch].uri.s);
	}

	/* Local Cancel */
	if(method != NULL && nReaon_Header == 0 && strlen(method) == 6 && !strncmp(method,"CANCEL",6) && Trans->nRoutingPlanCase == 1 && (!strncmp(cRequestUri,"sip:oms-",8) || !strncmp(cRequestUri,"sip:gext-",9) || !strncmp(cRequestUri,"sip:xfer-",9) || !strncmp(cRequestUri,"sip:opr-",8))) {
		*len += LOCAL_CANCEL_LEN + CRLF_LEN;
		nLocal_Header = 1;
	}
	/* Content Length, EoM */
	*len+=CONTENT_LENGTH_LEN+1 + CRLF_LEN + CRLF_LEN;
	...
	if(nReaon_Header)
	{
		append_mem_block(p,REASON CRLF ,REASON_LEN+CRLF_LEN);
	}

	/*Adding new header in CANCEL rqst to differentiate the MSICP local CANCEL or user CANCEL to 'fix abandoned calls(caller terminated call) issue in hunt group report(Routing plan to HUNT group --- Ring time out case)' --Prashant/Abhilash --Aug-2020*/
	if(nLocal_Header){ 
		append_mem_block(p,LOCAL_CANCEL CRLF ,LOCAL_CANCEL_LEN+CRLF_LEN);
	}
				...
				str* to, unsigned int *len, str *next_hop)
{
		...
		char cseq_temp[256]="",cseq_buff[256]="";
		char *tmp_ptr = NULL;
		int num_cseq=0;
		str cseq_append;
		...
		*len = SIP_VERSION_LEN + BYE_LEN + 2 /* spaces */ + CRLF_LEN;
		*len += ruri.len;
		...
		*len+= via_len;
		/*headers*/
		*len += Trans->from.len + Trans->callid.len + to->len + Trans->cseq_n.len + 1 + BYE_LEN + CRLF_LEN;
		...
		*len += CONTENT_LENGTH_LEN + 1 + CRLF_LEN + CRLF_LEN;
		...
		append_mem_block( p, BYE, BYE_LEN );
		append_mem_block( p, " ", 1 );
		append_str(p, ruri);
		append_mem_block( p, " " SIP_VERSION CRLF, 1 + SIP_VERSION_LEN + CRLF_LEN);
		...
		/* Added below for fixing group call at a time answering issue --vravi 23062011 */	
		snprintf(cseq_temp,sizeof(cseq_temp)-1,"%.*s",Trans->cseq_n.len, Trans->cseq_n.s);
		tmp_ptr = strchr(cseq_temp,' ');
		if(tmp_ptr){
			*tmp_ptr='\0';
			 tmp_ptr++;
			 if(tmp_ptr){
			 	num_cseq= atoi(tmp_ptr);
			 	num_cseq =num_cseq + 1; /* Incrementing cseq number fixing group call same time answer bug --vravi 23062011 */
				snprintf(cseq_buff,sizeof(cseq_buff)-1,"%s %d",cseq_temp,num_cseq);
				cseq_append.s = cseq_buff;
				cseq_append.len = strlen(cseq_buff);
				LOG(L_ERR, "!!!! ORIGNAL CSEQ HEADER LENGTH : %d  and FRAMED CSEQ HEADER LENGTH : %d  !!!!!!!\n",Trans->cseq_n.len,cseq_append.len );
				append_str(p, cseq_append);
			}else {
				LOG(L_ERR, "ERROR IN INCREMENTING THE NEW CSEQ NUMBER IN HEADER, FOLLOWING THE ORIGINAL  !!!!!!!\n");
				append_str(p, Trans->cseq_n);
			}
		}else {
			LOG(L_ERR, "ERROR IN PARSING THE ORIGINAL  CSEQ HEADER ,FOLLOWING THE ORIGINAL !!!!!!!\n");
				append_str(p, Trans->cseq_n);
		}
		append_mem_block( p, " ", 1 );
		append_mem_block( p, BYE, BYE_LEN);
