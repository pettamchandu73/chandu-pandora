/* 
 * $Id: my_con.c,v 1.1.1.1 2006/08/31 22:40:52 hari Exp $
 *
 *
...
int get_reconnection(struct my_con* con)
{
	if(!con || !con->id){
		LOG(L_ERR, "get_reconnection(): Connection or ID NULL \n");
		return 0;
	}
	...
	con->con = (MYSQL*)pkg_malloc(sizeof(MYSQL));
	if (!con->con) {
		LOG(L_ERR, "new_connection(): No enough memory\n");
		return 0;
	}
	...
	mysql_init(con->con);
	if (!mysql_real_connect(con->con, con->id->host.s,con->id->username.s, con->id->password.s,con->id->database.s,con->id->port, 0, 0)) {
		LOG(L_ERR, "new_connection(): %s\n", mysql_error(con->con));
		mysql_close(con->con);
		return 0;
	}
