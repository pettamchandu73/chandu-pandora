/* $Id: nathelper.c,v 1.1.1.1 2006/08/31 22:40:52 hari Exp $
 *
 ...
 */
 ...
 /*! Modification History XXX Take Care XXX 
  * 	2006-06-10 		Enabled the addition of extra lump in force_rtp_proxy2_f
  * 					if the packet is already proxied
  * 	2006-06-11		Added code for removing the multiple CRLF force_rtp_proxy2_f 
  * */
...
#define	ANORTPPROXY	"a=pandoranat:yes\r\n"
...
force_rtp_proxy2_f(struct sip_msg* msg, char* str1, char* str2)
{
...
#ifndef WS_BLOCK
	struct lump* anchor;
	int  crfound=0, lffound=0, crlffound = 0 ;
	int  actualsdplen;
#endif
	...
	if ( !sdp_1918(msg) ) {
		LOG(L_WARN, "Public Address in SDP");
		real = 1;
	}
	...
	LOG(L_ERR, "\nforce_rtp_proxy2_f altip:%s alt port:%s\n",newip.s,newport.s);
	...
	if (proxied == 0) {
		...
		/*! Following Block added for worksmart as we saw 
		 * some UA's adding multiple CRLF*/
		sdpbodylen = body.len;
		actualsdplen = sdpbodylen; 
		while ( sdpbodylen > 0 ) {
			if ( body.s [sdpbodylen-1] == '\r') {
				sdpbodylen--;
				crlffound = 1;
				crfound = 1;
			}
			else if(body.s[sdpbodylen-1]=='\n'){
				sdpbodylen--;
				crlffound = 1;
				lffound = 1; 		
			}
				...
				if(crlffound){	
					if(crfound && lffound ){	
						body.len = sdpbodylen + 2;
					}else if(crfound || lffound ){
						body.len = sdpbodylen + 1;
					}
					break;
				}
				sdpbodylen--;
		...
		if( body.len > actualsdplen ){
		/*There is some  thing wrong in that ----*/	
			body.len = actualsdplen	;
		}
		LOG(L_ERR, "Nathelper  %d  %d %d  %d %d\n", body.len, actualsdplen, crlffound, crfound, lffound);
		anchor = anchor_lump(msg, body.s + body.len - msg->buf, 0, 0);
		if (anchor == NULL) {
