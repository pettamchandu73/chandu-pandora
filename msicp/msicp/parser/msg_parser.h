/*
 * $Id: msg_parser.h,v 1.1.1.1 2006/08/31 22:40:56 hari Exp $
 *
...
struct sip_uri {
	...
	str method;
	str lr;
	str r2; /* msicp specific rr parameter */
	/* values */
	str transport_val;
	str ttl_val;
	str user_param_val;
...
typedef struct extra_call_info {
    // This is the new structure taken to communicate the details from worksmart module to racc module for billing purpose
             int nOtherDataCenterFlag; /*Added for blocking billing  */
}EXTRACALLINFO;
...
struct sip_msg {
	...
	*/
	...
	str set_global_port;
    EXTRACALLINFO extra_info;
	struct socket_info* force_send_socket; /* force sending on this socket,
											  if msicp */
	int nRoutingPlanExtFlag;	   /* Added to impliment routing plan feature: "Allow extension from PSTN phone field" */ 
    char cAgentId[256];
    char cGrpName[256];
