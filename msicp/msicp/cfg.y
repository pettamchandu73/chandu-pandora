/*
 * $Id: cfg.y,v 1.2 2006/09/17 07:13:59 hari Exp $
 *
 *  cfg grammar
 *
...
%token CHILDREN
%token CHECK_VIA
%token WSDB_FIFO_COUNT
%token WS_DUPLICATE_ROUTENO
%token WS_CTC_CALL_FLAG
%token WS_ACC_DB_USER
%token WS_ACC_DB_HOST
%token WS_ACC_DB_PSWD
%token WS_ACC_DB_NAME
%token WS_DB_FIFO_NAME
%token SYN_BRANCH
%token MEMLOG
%token SIP_WARNING
%token FIFO
...
assign_stm:	DEBUG EQUAL NUMBER { debug=$3; }
		...
		| WS_DUPLICATE_ROUTENO EQUAL NUMBER { ws_duplicate_routeno=$3; }
		| WS_DUPLICATE_ROUTENO EQUAL error { yyerror("int value expected"); }
		| WS_CTC_CALL_FLAG EQUAL NUMBER { ws_ctccall_flag=$3; }
		| WS_CTC_CALL_FLAG EQUAL error { yyerror("int value expected"); }
		| WSDB_FIFO_COUNT EQUAL NUMBER { wsdb_fifo_count=$3; }
		| WSDB_FIFO_COUNT EQUAL error { yyerror("int value expected"); }
		| WS_ACC_DB_USER EQUAL STRING { ws_acc_db_user=$3; }
		| WS_ACC_DB_USER EQUAL error { yyerror("string value expected"); }
		| WS_ACC_DB_HOST EQUAL STRING { ws_acc_db_host=$3; }
		| WS_ACC_DB_HOST EQUAL error { yyerror("string value expected"); }
		| WS_ACC_DB_PSWD EQUAL STRING { ws_acc_db_pswd=$3; }
		| WS_ACC_DB_PSWD EQUAL error { yyerror("string value expected"); }
		| WS_ACC_DB_NAME EQUAL STRING { ws_acc_db_name=$3; }
		| WS_ACC_DB_NAME EQUAL error { yyerror("string value expected"); }
		| WS_DB_FIFO_NAME EQUAL STRING { ws_db_fifo_name=$3; }
		| WS_DB_FIFO_NAME EQUAL error { yyerror("string value expected"); }
