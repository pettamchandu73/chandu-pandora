/* 
 * $Id: my_pool.c,v 1.1.1.1 2006/08/31 22:40:52 hari Exp $
 *
 *
 ...
 */
struct my_con* get_connection(const char* url, int reconn_flag)
{
	struct my_id* id;
	struct my_con* ptr;
	int pid, try_reconnection = 0;
	...
	if (try_reconnection == 1 && ptr){
		LOG(L_ERR, "Need Reconnection !!!!  \n");
		if(!get_reconnection(ptr)){
			LOG(L_ERR, "Failed to re-connect to db ... \n");
			return 0;
		}
		return ptr;
	}else{
		...
		if (!ptr) {
			LOG(L_ERR, "Unable to Create new connection !!!\n");
			free_my_id(id);
			return 0;
		}
		ptr->next = pool;
