/*
 * $Id: main.c,v 1.4 2007/03/07 10:07:55 hari Exp $
 *
...
#include "nestedshmclear/nested_call_shm.h"
...
static char id[]="@(#) $Id: main.c,v 1.4 2007/03/07 10:07:55 hari Exp $";
static char* version=SER_FULL_VERSION;
static char* flags=SER_COMPILE_FLAGS;
char compiled[]= __TIME__ " " __DATE__ ;


/*! The following variable are added for fifo based accounting of Worksmart */
	/*! Global variable to hold fifo handle */
	/*! Default fifo children count */
	int 	wsdb_fifo_count = 0;
	int 	ws_duplicate_routeno = 0;
	char *ws_acc_db_user;
	char *ws_acc_db_host;
	char *ws_acc_db_pswd;
	char *ws_acc_db_name;
	char *ws_db_fifo_name;
	/*! Name of the fifo node */
...
int ws_ctccall_flag = 0;			/* For enabling the ctc call signalling changes */
#ifdef USE_TCP
...
/* should msicp try to locate outbound interface on multihomed
...
void cleanup(show_status)
{
	...
	if(wsdb_fifo_count > 0){
		LOG(L_INFO,"killing the wsdb FIFO reader\n");
		system("pkill -9 msicpfiforeader");
	}
	destroy_pkg_mallocs();
}
 ...
 * (e.g. when parsing the config file) or when msicp is started in "dont-fork"
...
//	sprintf (command,"id=`cat /usr/local/etc/msicp/shm.id`\r\nipcrm -m $id");	
...
void handle_sigs()
{
	...
	switch(sig_flag){
		...
		case SIGTERM:
			...
			LOG(L_INFO," SIGTERM cleanup is succeeded\n");
		...
		case SIGCHLD:
			...
			cleanup(1); /* cleanup & show status*/
			LOG(L_INFO,"handle_sigs() cleanup is succeeded\n");
			alarm(0);
...
int main_loop()
{
	...
	if (dont_fork){
		...
		*/
		...
		if(fork() == 0) {
		 	LOG(L_WARN, "nested_calls_shm_clear Listener process\n");
		   	return nested_calls_shm_clear();
		}
...
int main(int argc, char** argv)
{
...
#ifdef DBG_MSG_QA
	fprintf(stderr, "WARNING: msicp startup: "
		"DBG_MSG_QA enabled, msicp may exit abruptly\n");
#endif
	...
	"s:"
#endif
	"f:cm:b:M:l:n:N:rRvdDETVhw:t:u:g:P:G:i:x:";
	...
	while((c=getopt(argc,argv,options))!=-1){
		switch(c){
			...
			case 'm':
					...
					LOG(L_INFO, "msicp: shared memory: %ld bytes\n",
									shm_mem_size );
					break;
					/* Added for pkg memory size change from server options --vravi */
			case 'M':
					pkg_mem_size=strtol(optarg, &tmp, 10) * 1024 * 1024;
					if (tmp &&(*tmp)){
						LOG(L_ERR, "bad pkg memory size number: -M %s\n",optarg);
						goto error;
					};
					LOG(L_NOTICE, "ser: pkg memory: %ld bytes\n",pkg_mem_size );
					break;
			case 'b':
...
try_again:
	...
	if(wsdb_fifo_count < 1){
		LOG(L_ERR,"Number of wsdb FIFO processes are :%d\n",wsdb_fifo_count);
		goto error;
	}
	bzero(wsdb_fifocmd,sizeof(wsdb_fifocmd));
	sprintf(wsdb_fifocmd,"/home/wsadmin/wsmsicp/msicpfiforeader/msicpfiforeader");
	LOG(L_INFO,"wsdb_fifocmd : %s\n",wsdb_fifocmd);
	...
	system(wsdb_fifocmd);
	LOG(L_INFO,"msicpfiforeader is started\n");
	print_rl();
