/*
 * $Id: tm.c,v 1.1.1.1 2006/08/31 22:40:48 hari Exp $
 *
 * TM module
 *
 *
 ...
 * * msicp's fathers, who brought everywhere the wind  *
...
static int w_t_get_tran(struct sip_msg* p_msg,struct sip_msg* msg, int *code);
static int w_t_save_stir_details_to_msg(struct sip_msg* p_msg, int *nFlag, char *cData);
...
static cmd_export_t cmds[]={
	{"t_newtran",          w_t_newtran,             0, 0,
			REQUEST_ROUTE},
	{"t_get_tran",		w_t_get_tran,		2, 0,
			ONREPLY_ROUTE},
	{"t_save_stir_details_to_msg", w_t_save_stir_details_to_msg,	2, 0,
			ONREPLY_ROUTE},
	...
	{"t_write_unix",      t_write_unix,             2, fixup_t_write,
	                REQUEST_ROUTE | FAILURE_ROUTE },
    {"t_terminate_duplicate", w_t_terminate_duplicate, 1, 0, ONREPLY_ROUTE },
...
static param_export_t params[]={
	...
	{"cancel_retransmission_flag",INT_PARAM, &cancel_retransmission_flag     },
	{0,0,0}
};
...
static int w_t_get_tran(struct sip_msg* p_msg,struct sip_msg* msg, int *code)
{
	return t_get_tran(p_msg, msg, code);
}

static int w_t_save_stir_details_to_msg(struct sip_msg* p_msg, int *nFlag, char *cData)
{
	return t_save_stir_details_to_msg(p_msg, nFlag, cData);
}
