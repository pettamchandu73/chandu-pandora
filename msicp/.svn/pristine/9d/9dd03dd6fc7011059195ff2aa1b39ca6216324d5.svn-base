/*
 * $Id: t_lookup.c,v 1.1.1.1 2006/08/31 22:40:48 hari Exp $
 *
...
/*Added for Worksmart */
 extern int ws_no_stateless_fwd;
...
int t_reply_matching( struct sip_msg *p_msg , int *p_branch )
{
	...
	short is_cancel;
	 ...
	 /*added for BYE problem if both are accepting in the same time*/
	//char * ws_msgbuffer;
	...
	*/
	...
	/*	if(is_invite(p_cell) &&  p_msg->REPLY_STATUS>=200 &&  p_msg->REPLY_STATUS < 300  && is_cancel == 0){
				if(p_cell->cell_status == WS_DLG_CONFIRMED ){
						if(p_cell->dialog_branch != branch_id ){
								LOG(L_ERR , "Already confirmed dialog  Code %d isinvite %d\n", p_msg->REPLY_STATUS,  is_invite(p_cell));
								ws_msgbuffer = build_ack(p_msg, p_cell, branch_id, &ws_msg_len);
								if (ws_msgbuffer) {
										uac =  &p_cell->uac[branch_id];
										SEND_PR_BUFFER(&uac->request, ws_msgbuffer, ws_msg_len);
										shm_free(ws_msgbuffer);

								}
								ws_msgbuffer = build_bye(p_msg, p_cell, branch_id, &ws_msg_len);
								if (ws_msgbuffer) {
										uac =  &p_cell->uac[branch_id];
										SEND_PR_BUFFER(&uac->request, ws_msgbuffer, ws_msg_len);
										shm_free(ws_msgbuffer);
								}
								ws_no_stateless_fwd=1;
								goto wsnomatch;

						}
				}else{
						p_cell->dialog_branch=branch_id;
						p_cell->cell_status = WS_DLG_CONFIRMED;
				}
		}*/
...
static inline void init_new_t(struct cell *new_cell, struct sip_msg *p_msg)
{
	...
	if (p_msg->REQ_METHOD==METHOD_CANCEL) new_cell->cancel_flag |= T_IS_CANCEL_FLAG;
...
int t_newtran( struct sip_msg* p_msg )
{
	...
	*/
	...
	if(p_msg != NULL && (p_msg->nRoutingPlanExtFlag == 1 || p_msg->nRoutingPlanExtFlag == 2 || p_msg->nRoutingPlanExtFlag == 3)) {
		T->nRoutingPlanCase = 1;
		LOG(L_ERR, "[t_newtran] setting 'nRoutingPlanCase' variable on cell channel T:'%p' p_msg->nRoutingPlanExtFlag:'%d' \n", T, p_msg->nRoutingPlanExtFlag);
	}
	return 1;
